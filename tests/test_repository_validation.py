...
def test_p1_6_assessment_linked_reasoning_contracts():
    ...
    for path in sorted((ROOT / "app" / "data").glob("*.json")):
        ...
        for item in cases:
            ...
            assert item.get("part") in part_ids, (
                f"{path.name}: invalid assessment part {item.get('part')}"
            )
            assert item.get("targetDomain") in (item.get("options") or []), (
                f"{path.name}: targetDomain must be an option"
            )
            assert len(item.get("options") or []) >= 3, (
                f"{path.name}: use at least three assessment options"
            )
            for field in (
                "rationale",
                "observe",
